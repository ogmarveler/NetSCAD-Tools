...
namespace NetGenCAD.Designer.Repositories
{
    ...
    public class SolidDimensions : IScadObject
    {
        ...
        public int Resolution { get; set; } = 360; // Default resolution for curves
        ...
        public string SurfaceFilePath { get; set; } = string.Empty; // New property for surface file path
    ...
    public static class SolidDimensionsExtensions
    {
        ...
        // ✅ Cache SQL strings as static readonly
        private static class SqlQueries
        {
            public static readonly string GetByName = "SELECT * FROM SolidDimensions WHERE Name = @Name ORDER BY OperationType ASC, CreatedAt ASC";
            public static readonly string GetByNameWithAxis = @"
                SELECT sd.*, ad.OSCADMethod as AxisOSCADMethod
                FROM SolidDimensions sd
                LEFT JOIN AxisDimensions ad ON sd.AxisDimensionsId = ad.Id
                WHERE sd.Name = @Name
        ...
        {
            return await connection.QueryAsync<SolidDimensions, string, SolidDimensions>(
                SqlQueries.GetByNameWithAxis,
                (solidDim, axisMethod) =>
                {
                    solidDim.AxisOSCADMethod = axisMethod;
                    return solidDim;
        ...
        public static async Task UpdateColorAsync(this SolidDimensions module, SqliteConnection connection, string color)
        {
            const string sql = @"
