...
namespace NetGenCAD.Designer.Utility
{
    /// <summary>
...
""@
        ...
        private static string FindOpenScadExecutable()
        {
            ...
            {
                // Prioritize nightly if installed (apt/snap), then stable
                var paths = new[] { "/usr/bin/openscad-nightly", "/usr/bin/openscad", "/usr/local/bin/openscad" };
                return paths.FirstOrDefault(File.Exists) ?? "openscad";  // Falls to PATH
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                var paths = new[]
            ...
            "/Applications/OpenSCAD.app/Contents/MacOS/OpenSCAD"
                ...
                return paths.FirstOrDefault(File.Exists) ?? "/Applications/OpenSCAD.app/Contents/MacOS/OpenSCAD";
            }

            return "openscad";  // Fallback
        }
    }
}
